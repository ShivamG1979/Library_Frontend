...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Login request
            const res = await axios.post('https://library-backend-evox.onrender.com/api/auth/login', { email, password });
            localStorage.setItem('token', res.data.token);

            // Fetch user profile to get the username
            const config = { headers: { 'x-auth-token': res.data.token } };
            const profileRes = await axios.get('https://library-backend-evox.onrender.com/api/auth/profile', config);
